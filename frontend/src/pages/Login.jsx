...
import { login } from "../api"; // 🛰️ Fixed: Path adjusted to find src/api.js from src/pages/
import { useNavigate, Link } from "react-router-dom";
import { 
  Lock, Mail, AlertCircle, Zap, 
  Terminal, Activity, Radio, Cpu 
} from "lucide-react";
...
export default function Login({ setToken }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Handle backend errors (e.g., Personnel not found, Invalid Password)
      if (res?.error) {
        setMsg(res.error);
        setIsSubmitting(false);
      } else if (res?.token) {
        // Store the Tactical Passport (JWT) in local storage for persistence
        localStorage.setItem("token", res.token);
        
        // Pass token to App state to unlock private routes
        if (setToken) setToken(res.token);
        
        // Signal Locked. Proceed to Dashboard.
        nav("/dashboard");
      } else {
      ...
      setMsg("AUTH_SEQUENCE_FAILED: Signal Lost");
      setIsSubmitting(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#020202] font-mono text-white flex items-center justify-center p-6 relative overflow-hidden selection:bg-cyan-500 selection:text-black">
      ...
      {/* --- HUD OVERLAYS: CRT SCANLINE & GRID --- */}
      ...
      >
        ...
        <div className="mb-10 flex justify-between items-end border-b border-white/5 pb-6">
            ...
            <div className="text-right hidden sm:block">
                ...
                <div className="text-[8px] font-bold text-zinc-600">LOCAL_GRID_ACTIVE</div>
            </div>
        </div>
        ...
        <div className="relative group">
            ...
            <div className="bg-zinc-950/80 backdrop-blur-3xl border border-white/5 p-10 shadow-[0_0_100px_rgba(0,0,0,1)]">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <AnimatePresence>
                  {msg && (
                    ...
                    >
                      <AlertCircle size={14} className="animate-pulse text-red-500" /> {`Protocol_Err: ${msg}`}
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <button 
                  ...
                  className="group relative w-full bg-white text-black py-5 font-black uppercase italic tracking-widest text-xs hover:bg-zinc-200 transition-all flex items-center justify-center gap-4 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="mt-12 text-center flex justify-between items-center px-2 border-t border-white/5 pt-8">
      ...
      {/* FOOTER HUD DECOR */}
      <div className="fixed bottom-10 left-10 text-zinc-800 text-[9px] font-black space-y-2 hidden lg:block tracking-widest uppercase">
        ...
        <div className="text-white opacity-40 animate-pulse">SYSTEM_ACTIVE // VER_4.0_JWT</div>
      </div>
    </div>
  );
}
